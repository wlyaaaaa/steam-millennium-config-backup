...
> [!NOTE]
> 为了保护隐私并避免将敏感凭据/Token 提交到 Git 仓库，备份白名单仅包含主题与插件的 manifest 描述文件（如 `plugin.json`/`metadata.json`），而不包含插件在各自子目录中生成的自定义私有设置文件（如 `config.json`）。如果这些插件有自定义配置，恢复后需要手动重新配置。
